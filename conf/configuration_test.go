...
var _ = Describe("Configuration", func() {
	...
	Describe("Cast media base URL", func() {
		It("is empty by default", func() {
			conf.Load(true)
			Expect(conf.Server.CastMediaBaseURL).To(BeEmpty())
		})

		It("accepts a receiver-accessible HTTP URL", func() {
			viper.Set("castmediabaseurl", "http://192.168.2.11:4533")
			conf.Load(true)
			Expect(conf.Server.CastMediaBaseURL).To(Equal("http://192.168.2.11:4533"))
		})

		It("rejects a URL without an HTTP scheme", func() {
			viper.Set("castmediabaseurl", "192.168.2.11:4533")
			Expect(func() { conf.Load(true) }).To(PanicWith(ContainSubstring("CastMediaBaseURL")))
		})
	})
