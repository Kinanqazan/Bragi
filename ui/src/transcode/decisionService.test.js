...
describe('decisionService', () => {
  ...
  describe('getDecision', () => {
    it('shares one pending request between concurrent callers', async () => {
      ...
      const first = service.getDecision('song-1', fakeProfile)
      const second = service.getDecision('song-1', fakeProfile)
      expect(first).toBe(second)
      await Promise.resolve()
      expect(mockFetchFn).toHaveBeenCalledTimes(1)

      resolveDecision(makeFakeDecision())
      await expect(first).resolves.toBeTruthy()
      expect(await service.getDecision('song-1', fakeProfile)).toBeTruthy()
      expect(mockFetchFn).toHaveBeenCalledTimes(1)
    })

    it('removes a failed pending request so it can be retried', async () => {
      mockFetchFn
        .mockRejectedValueOnce(new Error('temporary failure'))
        .mockResolvedValueOnce(makeFakeDecision())

      await expect(service.getDecision('song-1', fakeProfile)).rejects.toThrow(
        'temporary failure',
      )
      await expect(
        service.getDecision('song-1', fakeProfile),
      ).resolves.toBeTruthy()
      expect(mockFetchFn).toHaveBeenCalledTimes(2)
    })

    it('does not cache a request invalidated while it is pending', async () => {
      let resolveDecision
      mockFetchFn.mockImplementationOnce(
        () =>
          new Promise((resolve) => {
      ...
      const request = service.getDecision('song-1', fakeProfile)
      await Promise.resolve()
  ...
  describe('prefetchDecisions', () => {
    ...
    it('limits background prefetching to one request at a time', async () => {
      let active = 0
      let maxActive = 0
      mockFetchFn.mockImplementation(async () => {
        ...
        await Promise.resolve()
        active -= 1
        return makeFakeDecision()
      })

      await Promise.all([
        service.prefetchDecisions(['song-1', 'song-2'], fakeProfile),
        service.prefetchDecisions(['song-3', 'song-4'], fakeProfile),
      ])
