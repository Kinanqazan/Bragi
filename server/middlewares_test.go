...
var _ = Describe("middlewares", func() {
	...
	Describe("corsHandler", func() {
		It("reflects the requesting origin instead of using a wildcard", func() {
			next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				w.WriteHeader(http.StatusOK)
			})
			req := httptest.NewRequest(http.MethodGet, "/rest/stream", nil)
			req.Header.Set("Origin", "https://navidrome.lan")
			rec := httptest.NewRecorder()

			corsHandler()(next).ServeHTTP(rec, req)

			Expect(rec.Header().Get("Access-Control-Allow-Origin")).To(Equal("https://navidrome.lan"))
		})

		It("supports Cast media preflight headers", func() {
			next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				w.WriteHeader(http.StatusOK)
			})
			...
			req.Header.Set("Access-Control-Request-Headers", "Range, Accept-Encoding")
			rec := httptest.NewRecorder()
			...
			Expect(rec.Header().Get("Access-Control-Allow-Origin")).To(Equal("https://navidrome.lan"))
			Expect(rec.Header().Get("Access-Control-Allow-Headers")).To(ContainSubstring("Range"))
			Expect(rec.Header().Get("Access-Control-Allow-Headers")).To(ContainSubstring("Accept-Encoding"))
		})
	})
